...
const validateProfileForm = (developer: Developer) => {
  ...
  if (!trimmedData.firstName) {
    ...
    trimmedData.firstName.length < 2 ||
    trimmedData.firstName.length > 50
  ) {
  ...
  if (!trimmedData.lastName) {
    ...
    trimmedData.lastName.length < 2 ||
    trimmedData.lastName.length > 50
  ) {
  ...
  if (trimmedData.githubLink) {
    ...
    if (!urlRegex.test(trimmedData.githubLink)) {
      errors.githubLink =
        "GitHub link must be a valid URL starting with http:// or https://";
    }
  }
  ...
  if (!trimmedData.hourlyRate) {
    ...
    errors.hourlyRate =
      "Hourly rate must be a number with up to 2 decimal places";
  }
